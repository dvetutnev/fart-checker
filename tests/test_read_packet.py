...
async def test_normal():
    ...
    port.readByte.side_effect = [b"\xFF", b"\x86", b"\x00", b"\x03", b"\x00", b"\x00", b"\x00", b"\x00", b"\x77"]

    assert await gas_sensor.readPacket(port) == b"\xFF\x86\x00\x03\x00\x00\x00\x00\x77"
    assert port.readByte.await_count == 9


@pytest.mark.asyncio
async def test_invalid_checksum():
    port = AsyncMock(aserial.ASerial)
    port.readByte.side_effect = [b"\xFF", b"\x86", b"\x00", b"\x03", b"\x00", b"\x00", b"\x00", b"\x00", b"\xAA"]
